...
pub fn message_url(chat: &Chat, msg_id: MessageId) -> Option<Url> {
    Message::url_of(chat.id, chat.username(), msg_id)
}

pub fn message_public_url(chat: &Chat, msg_id: MessageId) -> Option<Url> {
...
pub fn user_url(user: &User) -> Option<Url> {
    user.tme_url()
}
...
impl<'a> MessageText<'a> {
    ...
    pub fn append_text_link(&mut self, link_text: impl AsRef<str>, link: Url) {
        self.append_text_with_entity(link_text, MessageEntityKind::TextLink { url: link });
    }

    pub fn prepend_text_link(&mut self, link_text: impl AsRef<str>, link: Url) {
        self.prepend_text_with_entity(link_text, MessageEntityKind::TextLink { url: link });
    }

    pub fn append_text_link_fallback(&mut self, link_text: impl AsRef<str>, link: Option<Url>) {
        match link {
...
impl<'a> MessageTextBuilder<'a> {
    ...
    pub fn link(mut self, text: impl AsRef<str>, url: Url) -> Self {
    ...
    pub fn preview(mut self, enable: bool) -> Self {
        self.text.disable_preview = !enable;
        self
    }
